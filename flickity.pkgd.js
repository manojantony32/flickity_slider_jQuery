...
Flickity.prototype.select = function( index, isWrap ) {
  ...
  if ( this.cells[ index ] ) {
    ...
    console.log('select');
    //getting selected date and month

    var selectedDate = $('.gallery-cell.is-selected').find('li:eq(0)').attr('date');
    var selectedStatus = $('.gallery-cell.is-selected').find('li:eq(1)').attr('status');
    ...
    */
    ...
    var year = dayStart.getFullYear();
    var month = dayStart.getMonth() + 1;*/
    var dateToDb = selectedDate;
    /*var currentDate = $('.gallery-cell.is-selected').find('li:eq(1)').html();
    console.log(currentDate)
    dateToDb = dateToDb.split('-');
    dateToDb[2] = currentDate;
    dateToDb = dateToDb[0] + '-' + dateToDb[1] + '-' + dateToDb[2];*/
    //console.log(dateToDb)
    //angular.element(document).ready(function () {
      //console.log(window.addTopupWindow)
    var setJSONData = {};
    /*if (window.localStorage['monthTopUpData'] != undefined) {
      setJSONData = JSON.parse(window.localStorage['monthTopUpData']);      
    } else {
      debugger*/
      setJSONData = staticTruTime;
    //}
    if(dateToDb == undefined){
      dateToDb = "2017-08-10";
    }
    //if(window.addTopupWindow != undefined)      
    window.addTopupWindow(dateToDb, setJSONData);
    //});

    //window.loadSpinnerExt();    
    //scopeHolder(dateToDb);
    /*console.dir(window.loadSpinnerExt)
    if(window.loadSpinnerExt != undefined)
    window.loadSpinnerExt();*/
};
...
Unidragger.prototype.hasDragStarted = function( moveVector ) {
  return Math.abs( moveVector.x ) > 4 || Math.abs( moveVector.y ) > 4;
};
